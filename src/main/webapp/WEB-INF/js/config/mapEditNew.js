...
var Ploy={
    ...
    getData1:function(){
        ...
        var el=$('div.'+options.cssClickClass1);
    ...
    //添加点在地图区域中
    addPoint1:function(top,left){

        var wapper1=$(options.wapper1);

        var t=wapper1.offset().top;

        var l=wapper1.offset().left;

        var pt=top-t;

        var pl=left-l;

        var point=$('<div/>').addClass(options.cssClickClass1)
    ...
    makeRect:function(el,o){
        ...
        var datas=s.getData();
        if(datas.length == 2){
        	var gyr1=$(points[0]).position();
        	var gyr2=$(points[1]).position();
    ...
    drawRect:function(left1,top1,left2,top2){
    	var temp;
    	//小坐标提到前位
    	if(left1>left2){
    		temp=left1;
        ...
        s.addPoint1(top1,left1);
        s.addPoint1(top2,left2);
        s.makeSvgContainer1();
        ...
        var datas=s.getData1();
        	...
        	var gyr1=$(points[0]).position();
        	var gyr2=$(points[1]).position();
//        	var minLeft=left1<left2?left1:left2;
//        	var minTop=top1<top2?top1:top2;
        	var pointTop1=0;
        	var	pointLeft1=0;
        	var	pointTop2=top2-top1;
        	var	pointLeft2=left2-left1;


            //生成路径
        	var path = "M"+(pointLeft1+left1)+","+(pointTop1+top1)
        				+" L"+(pointLeft2+left1)+","+(pointTop1+top1)
        				+" L"+(pointLeft2+left1)+","+(pointTop2+top1)
        				+" L"+(pointLeft1+left1)+","+(pointTop2+top1)
        				+" L"+(pointLeft1+left1)+","+(pointTop1+top1);

            var ploy = Raphael('mapPloy',pointLeft2+left1,pointTop2+top1);
            this.paper = ploy;
